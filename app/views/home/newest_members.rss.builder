...
xml.rss(:version=>"2.0") do
  xml.channel do
    xml.title "#{GlobalConfig.application_name} Newest Member Feed"
    xml.link GlobalConfig.application_url
    xml.description "This feed will quickly show who has recently signed up for #{GlobalConfig.application_name}"
    xml.language 'en-us'
    new_members.each do |newest_member|
      xml.item do
        xml.title "New member of #{GlobalConfig.application_name}: #{newest_member.f}"
