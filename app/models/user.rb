...
class User < ActiveRecord::Base
  ...
  has_many :my_feed_items, :through => :feeds, :source => :feed_item, :conditions => ["feed_items.creator_id=?", self.id], :dependent => :destroy
  
  def all_my_feed_items(limit = 20)
    FeedItem.find(:all, 
                  :joins => 'INNER JOIN feeds ON feeds.feed_item_id = feed_items.id', 
                  :conditions => ["feeds.ownable_type='User' AND feeds.ownable_id=? AND feed_items.creator_id=?", self.id, self.id], 
